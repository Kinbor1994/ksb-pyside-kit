...
class TextConfig(BaseWidgetConfig):
    """Configuration for text widgets."""

    text: str = ""
    icon: Optional[str] = None
    icon_size: Size = Field(default_factory=lambda: Size.px(16))
...
class FormFieldConfig(BaseWidgetConfig):
    ...
    def set_default_error_messages(
        ...
        default_messages = {
            ...
            "type": "Incorrect value type.",
        }
        if isinstance(value, dict):
            default_messages.update(value)
        return default_messages

    @model_validator(mode="after")
    def validate_read_only_state(self) -> "FormFieldConfig":
        """Validates read_only and required states."""
        if self.read_only and self.required:
            raise ValueError("read_only cannot be True when required is True.")
        return self
...
class TextFieldConfig(FormFieldConfig):
    ...
    def validate_length_constraints(
        cls, max_length: Optional[int], values: Dict[str, Any]
    ) -> Optional[int]:
    ...
    def validate_value_constraints(
        cls, max_value: Optional[float], values: Dict[str, Any]
    ) -> Optional[float]:
    ...
    def get_error_messages(self) -> Dict[str, str]:
        """Returns default and custom error messages."""
        messages = {
            "required": "This field is required.",
            "email": "Invalid email address.",
            "numeric": "Only numbers are allowed.",
            "pattern": "Invalid format.",
            "min_value": lambda x: f"The value must be greater than or equal to {x}",
            "max_value": lambda x: f"The value must be less than or equal to {x}",
            "min_length": lambda x: f"The text must be at least {x} characters long",
            "max_length": lambda x: f"The text must not exceed {x} characters long",
        }
